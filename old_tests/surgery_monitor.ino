 ...
 * PROJECT: Vital-signs monitor for small rodents (rat / mouse).
 * @version 1.0
...
//
//  If the envelope swing is below CALIB_MIN_SWING ADC counts, the signal is
...
static const int CALIB_MIN_SWING = 120;
...
//        drops to zero in a clearly breathing animal, lower THRESH_INSP_FRAC
//        toward 0.55 and THRESH_EXP_FRAC toward 0.35.
// ═══════════════════════════════════════════════════════════════════════════════

static const float THRESH_INSP_FRAC = 0.65f;
static const float THRESH_EXP_FRAC  = 0.45f;
...
static const float TEMP_MIN_C    = 30.0f;       // Below: probe disconnected or shorted
static const float TEMP_MAX_C    = 45.0f;       // Above: probe disconnected or shorted
                                                 // Normal rodent range: 36.5-38.5 C
...
static void recomputeThresholds()
{
    ...
    const int newInsp = (int)(vMin + THRESH_INSP_FRAC * (vMax - vMin));
    const int newExp  = (int)(vMin + THRESH_EXP_FRAC  * (vMax - vMin));
...
void loop()
{
    ...
    if (now - timerDisplay >= INTERVAL_DISPLAY_MS) {
        ...
        u8g2.setCursor(0, 31);
        u8g2.print(F("Temp: "));
        if (tempCelsius >= TEMP_MIN_C && tempCelsius <= TEMP_MAX_C) {
    ...
    // TASK 5 — TELEMETRY UPDATE (Non-blocking, 100 Hz out to Serial3)
    telemetry_update();
}

 ...
 * ═══════════════════════════════════════════════════════════════════════════════
 *
 * [T1] DECIM_RATIO = 1 (no software decimation)
 *   The Maxim spo2_algorithm.cpp FIR filter coefficients were derived for
 *   25 Hz input (i.e., 100 Hz FIFO with DECIM_RATIO=4). With DECIM_RATIO=1
 ...
 *   set DECIM_RATIO=4, and verify that heartRate detection still functions
 ...
 *   which restores full AC amplitude at 400 Hz. Adjust HR_MIN_BPM and
 *   HR_MAX_BPM to the rodent physiological window before doing this.
 *
 ...
 *   HR_MIN_BPM and HR_MAX_BPM are currently set for human bench testing.
...
//
//  HR_OUTLIER_FRAC: fraction by which a new beat may deviate from the rolling
...
static const float   HR_MIN_BPM      = 40.0f;    // Human bench: 40 BPM minimum
static const float   HR_MAX_BPM      = 200.0f;   // Human bench: 200 BPM maximum
static const float   HR_OUTLIER_FRAC = 0.35f;
...
// Lowering SPO2_MIN_VALID to 70 can be useful during diagnostic sessions to
// see algorithm output even from a degraded signal.
static const uint8_t SPO2_MIN_VALID  = 80;
...
// ═══════════════════════════════════════════════════════════════════════════════
//
//  DECIM_RATIO = 1: every FIFO sample (100 Hz) is written directly to the
...
//
//  If DECIM_RATIO is changed to 4: samples are averaged in groups of 4 before
...
// With DECIM_RATIO=1 at 100 Hz: buffer fills in 1 s, updates every 0.25 s.
// With DECIM_RATIO=4 at 25 Hz: buffer fills in 4 s, updates every 1 s.
...
// Decimation accumulators (used only when DECIM_RATIO > 1)
...
bool max30102_init()
{
    ...
    //
    //  ledBrightness = 80 (~16 mA per LED)
    //    Adequate for human fingertip with adcRange=16384.
    //    For rodent tail/paw (thinner tissue): may need reduction to 40-60
    //    to avoid saturation with adcRange=4096 or 2048.
    //
    //  sampleAverage = 4
    //    Hardware averages 4 raw conversions per FIFO entry.
    //    Effective FIFO output rate: 400 / 4 = 100 Hz.
    //    Smooths high-frequency electrical noise before the FIFO.
    //    Tradeoff: AC amplitude of the pulse waveform is reduced (see [T2]).
    //
    //  ledMode = 2
    //    Activates both Red (660 nm) and IR (880 nm) channels.
    //    Required for SpO2 (ratio of Red AC/DC to IR AC/DC = R value).
    //
    //  sampleRate = 400 Hz (internal, before hardware averaging)
    //
    //  pulseWidth = 215 us
    //    Shorter than maximum (411 us) → less shot noise per pulse.
    //    Adequate LED power for the current brightness setting.
    //
    //  adcRange = 16384
    //    Required for human fingertip to avoid ADC saturation at LED=80.
    //    Full-scale at 18-bit hardware = 262143 counts.
    //    Target DC level: 20-50% of full scale (52000-130000 counts).
    //    For rodent sites with lower absorption, consider reducing to 4096
    //    and verifying IR_raw in serial diagnostic output.
    //
    particleSensor.setup(
        80,      // ledBrightness
        4,       // sampleAverage → 100 Hz FIFO
        2,       // ledMode: Red + IR
        400,     // sampleRate (internal Hz)
        215,     // pulseWidth (us)
        16384    // adcRange
    );

    // Restore I2C clock — particleSensor.setup() may modify I2C registers.
    ...
    Serial.println(F("  LED=80 | sampleAvg=4 | FIFO=100Hz | DECIM=1 | adcRange=16384"));
...
static void _process_beat(unsigned long nowMs)
{
    ...
    // IMPORTANT: change HR_MIN_BPM / HR_MAX_BPM before animal use (see above).
    ...
    if (hrFill >= 4) {
        ...
        if (mean > 0.0f && fabsf(bpm - mean) / mean > HR_OUTLIER_FRAC) {
            hrRejectCount++;
...
static const bool PLOTTER_ENABLE = true;  // true = Plotter | false = Monitor
...
void max30102_update()
{
    ...
    while (particleSensor.available())
    {
        ...
        // ── PATH 2 — SpO2 (DECIM_RATIO = 1, effective 100 Hz to algorithm) ───
        // With DECIM_RATIO=1, sumRed/sumIR accumulate exactly one sample each,
        // and the division sumRed/1 is a no-op. The decimation structure is
        // retained so that DECIM_RATIO can be changed to 2 or 4 without
    ...
    // 1 Hz diagnostic output — active during development and bench validation.
    // Comment out for final deployment if Serial output is not needed.
    _debug_serial();
}
...
bool max30102_spo2Valid() { return (spO2 >= SPO2_MIN_VALID); }
// to get raw IR signal
